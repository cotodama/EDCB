...
<イベント情報>のリスト|nil SearchEpg( <検索条件> )
<イベント情報>|nil SearchEpg( ネットワークID:I, TSID:I, サービスID:I, イベントID:I )
  イベント情報を検索する
  1引数のときは<検索条件>にマッチしたイベントを取得する。
...
<検索条件>={
  ...
  network:I=対象ネットワーク(+1=地デジ,+2=BS,+4=CS,+8=その他)
  days:I|nil=対象期間(現時刻からdays*24時間以内)
  days29:I|nil=対象期間(現時刻からdays*29時間以内。互換用なので使用しないこと)
}
...
<自動予約検索条件>={
  andKey:S=キーワード
  notKey:S=NOTキーワード
  regExpFlag:B=正規表現モード
  titleOnlyFlag:B=番組名のみ検索対象にする
  aimaiFlag:B=あいまい検索モード
  notContetFlag:B=対象ジャンルNOT扱い
  notDateFlag:B=対象期間NOT扱い
  freeCAFlag:I=スクランブル放送(0=限定なし,1=無料のみ,2=有料のみ)
  chkRecEnd:B=録画済かのチェックあり
  chkRecDay:I=録画済かのチェック対象期間
  contentList:{
    content_nibble:I=大分類*256+中分類
  }のリスト=対象ジャンル
  dateList:{
